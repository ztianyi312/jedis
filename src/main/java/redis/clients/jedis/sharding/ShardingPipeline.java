...
public class ShardingPipeline extends Queable {
    private PoolSharding poolSharding;
    private List<FutureResult> results = new ArrayList<FutureResult>();
    private Queue<ExtendedJedisShardInfo> JedisShardInfos = new LinkedList<ExtendedJedisShardInfo>();
    private Map<ExtendedJedisShardInfo, Jedis> JedisMap = new HashMap<ExtendedJedisShardInfo, Jedis>();

    private ExtendedJedisShardInfo currentShardInfo;
    ...
    public void setShardedJedis(PoolSharding poolSharding) {
     ...
     */
    public void sync() {
        for (JedisShardInfo shardInfo : JedisShardInfos) {
            generateResponse(this.JedisMap.get(shardInfo).getClient().getOne()).get();
        }
    }
    ...
    public List<Object> syncAndReturnAll() {
        ...
        for (JedisShardInfo shardInfo : JedisShardInfos) {
            formatted.add(generateResponse(this.JedisMap.get(shardInfo).getClient().getOne()).get());
        }
    ...
    public void releaseResource() {
        for(Map.Entry<ExtendedJedisShardInfo, Jedis> entry : this.JedisMap.entrySet()) {
            poolSharding.getShard(entry.getKey()).returnResource(entry.getValue());
        } 
    }
    ...
    public ExtendedJedisShardInfo getShardInfo(String key) {
        currentShardInfo = poolSharding.getShardInfo(key);
        Jedis jedis = this.JedisMap.get(currentShardInfo);
        if(jedis == null) {
            jedis = poolSharding.getShard(currentShardInfo).getResource();
            this.JedisMap.put(currentShardInfo, jedis);
        }
        
        return currentShardInfo;
    }

    private Client getClient(String key) {
        ExtendedJedisShardInfo shardInfo = getShardInfo(key);
        JedisShardInfos.add(currentShardInfo);
        return this.JedisMap.get(currentShardInfo).getClient();
    }

    public ExtendedJedisShardInfo getCurrentShardInfo() {
        return currentShardInfo;
    }
